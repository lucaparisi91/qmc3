...
def registerJastrow(name,qmcName=None):
    if qmcName is None:
        qmcName=name
...
def createJastrow(qmcName,*args,**kwds):
    j = _registered_jastrows[qmcName](*args,**kwds)
...
class jastrow:
    ...
    def save(self,filename):
        with open(filename, 'w') as fp:
            ...
            parameters["kind"]=type(self).qmcName
...
@registeredJastrows("delta_bound_state_phonons")
...
@registeredJastrows("dipolar_rep")
...
@registeredJastrow("hardSphereGauss")
...
@registerJastrow("bSpline")
class splineJastrow (jastrow):
    
    inputParameters=["y","stepSize"]
    
    def __init__(self,x=None,y=None,derivativeRight=0,derivativeLeft=0):
        jastrow.__init__(self)
        self.parameters["y"]=y
        self.parameters["x"]=x
