...
void testChainsJoined( pimcConfigurations & configurations, int iChainLeft, int iChainRight)
{
    ...
    ASSERT_FALSE(chainLeft.hasHead() );
    ASSERT_FALSE(chainRight.hasTail()  );

    // check that the link is bidirectional
    ASSERT_EQ(chainLeft.next , iChainRight);
    ASSERT_EQ(chainRight.prev , iChainLeft);
    ...
    ASSERT_EQ(groupLeft.iStart , groupRight.iStart);
    ASSERT_EQ(groupLeft.iEnd , groupRight.iEnd);
